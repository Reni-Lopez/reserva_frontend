...
@Component({
  selector: 'app-new-owmer',
  templateUrl: './new-owmer.component.html',
  styleUrls: ['./new-owmer.component.css']
})
export class NewOwmerComponent implements OnInit {
  ...
  createOwner(){
    ...
    if (this.equalsPassword(this.owner.get('password').value,this.owner.get('confirm_password').value)) {
      this._ownerService.createOwner( this.ownerModel )
    .subscribe(
      ( resp : any) => {
        Swal.fire(
          'Comerciante Agregado',
          'Presiona OK',
          'success'
        );
        this.router.navigateByUrl('/lobby/list/owner' );
      },
      (error) => {
